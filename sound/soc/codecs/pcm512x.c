...
static int pcm512x_codec_probe(struct snd_soc_codec *codec)
{
    struct pcm512x_priv *pcm512x = dev_get_drvdata(codec->dev);
    
    codec->control_data = pcm512x->regmap;
...
static int pcm512x_mute(struct snd_soc_dai *dai, int mute)
{
	return 0;
}
...
static void pcm512x_shutdown(struct snd_pcm_substream *substream,
	struct snd_soc_dai *dai)
{
...
static int pcm512x_set_dai_clkdiv(struct snd_soc_dai *codec_dai, int div_id, int div)
{
	return 0;
}

static int pcm512x_set_dai_fmt(struct snd_soc_dai *codec_dai,
			       unsigned int fmt)
{
	return 0;
}

struct snd_soc_dai_ops pcm512x_dai_ops = {
	.startup = pcm512x_startup,
	.shutdown = pcm512x_shutdown,
	.hw_params = pcm512x_hw_params,
	.digital_mute = pcm512x_mute,
	.set_sysclk = pcm512x_set_dai_sysclk,
	.set_clkdiv = pcm512x_set_dai_clkdiv,
...
static struct snd_soc_dai_driver pcm512x_dai = {
	...
	.playback = {
		...
		.channels_min = 1,    /*support mono and stereo*/
		.channels_max = 2,
...
//	.ranges = &pcm512x_range,  /*3.4.x unsupport*/
//	.num_ranges = 1,                 /*3.4.x unsupport*/
...
int pcm512x_probe(struct device *dev, struct regmap *regmap)
{
	...
	pcm512x->sclk = clk_get(dev, NULL); /*3.4.x unsupported devm_clk_get()*/
